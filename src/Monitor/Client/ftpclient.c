...
int DecryptCommand(const char* recvBuf, const int decryptMode, char* tmpBuf)
{
...
int SendCommandAndReceiveAck(const int socketCtrlFd, const char** cmd, const char** param, char* cmdPromote)
{
    ...
    //接收服务端ACK
    memset(tmpRecvBuf, 0, BUFSIZE);
    do {
        if (fgets(tmpRecvBuf, BUFSIZE - 3, controlStream) == NULL) {
            printf("fgets error.\n");
            exit(-1);
        }
        // 解密命令：
...
int Login(const Option* options, SocketAddr* serverSocketAddrs)
{
    ...
    switch(ackCode){
        ...
        case ACK_331:
            ...
            //passwd
            if (strcasecmp(options->userName, "anonymous") == 0) {
                options->userPasswd = "123456";
            }
            //加密密码
            char tmpBuf[1024] = {};
