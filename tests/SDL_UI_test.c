...
int show_playtime(double x)
{
	int64_t ts;
	int ns, hh, mm, ss;
	
	tns = 123456789;//cur_stream->ic->duration/1000000LL;
	
	frac = x / progress_bar.w;
	ns = frac * tns;
	hh = ns / 3600;
	mm = (ns % 3600) / 60;
	ss = (ns % 60);
	fprintf(stderr, "Seek to %2.0f%% (%2d:%02d:%02d)  \n", frac * 100, hh, mm, ss);
	...
	char time_ttf[16] = {};
	sprintf(time_ttf, "%2d:%2d:%2d", hh, mm, ss);
	gTimeMessage = TTF_RenderText_Shaded(font,a,RGB_Black,RGB_White);
	SDL_SetAlpha(gTimeMessage, SDL_SRCALPHA|SDL_RLEACCEL, 128);   //设置半透明

	SDL_Surface time_rect;
	time_rect.x = x;
	time_rect.y = progress_bar.y - 10;
	time_rect.w = 32;
	time_rect.h = 10;
	
	SDL_BlitSurface(gTimeMessage, NULL, gScreenSurface, &time_rect);
	SDL_Flip(gScreenSurface);
...
static int prepare_button_rect(void)
{
